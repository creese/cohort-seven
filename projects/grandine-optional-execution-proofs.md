...
A recursive layer sits above the base execution proof. A prover folds
each payload's execution proof into a rolling `BeaconChainProof` that
...
to successfully receive, validate and retain execution proofs and the
recursive `BeaconChainProof` objects built above them. As such, any
execution client or Engine API integrations are currently excluded
from the core requirements and will only be pursued as stretch goals.
...
objects generated by external proof systems. The implementation
includes SSZ representation and hashing, payload context resolution
and binding validation, signature verification, cryptographic proof
verification for both proof layers, gossip reception, checkpoint and
verified-head anchor tracking, weak-subjectivity-to-head sync gating,
and proof state management. All proof distribution is gossip-based; Grandine
implements no proof request/response protocol.
...
flowchart TD
    ...
    Peer -->|Execution proof and<br> BeaconChainProof gossip| P2P
...
| `ProofEngine` | Verifies execution proofs and recursive `BeaconChainProof` objects, maintains stored proof state, and handles new payload and fork-choice notifications for retention and pruning |
...
proofs. The recursive layer adds a beacon-chain proof and its public
input. Together, these types form the core execution-proof data model
...
| `BeaconChainProof` | Recursive proof attesting that every payload across a beacon-chain range passed Engine API validation |
...
| Recursive proof SSZ containers | `types` |

This area defines the EIP-8025 SSZ containers `PublicInput`,
`ExecutionProof`, and `SignedExecutionProof`, along with the recursive
`BeaconChainProof` and its public input. It also supports their
...
| recursive proof verification interface | `ProofEngine` (proposed) |
...
interfaces for both proof layers, integrates proof verification with
...
anchor from the trusted checkpoint plus the next `BeaconChainProof`
...
submitted to `ProofEngine` for cryptographic verification. Recursive
proofs additionally require a matching trusted checkpoint and a head
known to the fork-choice store.
...
| beacon chain proof gossip topic and pubsub message type | <span style="white-space: nowrap;"><code>eth2_libp2p::types::{topics, pubsub}</code></span> |
...
Grandine will receive per-payload execution proofs and recursive
`BeaconChainProof` objects through gossip. Proofs will be routed to
the execution proof service for consensus-layer validation and
submission to `ProofEngine`, and re-published on success. A node
starting from a weak-subjectivity checkpoint waits for a
`BeaconChainProof` anchored at that checkpoint and adopts the head it
commits to as the node's verified head.
...
- **Network Simulation and Mock Proving via zkboost**: To validate the
  ...
  `BeaconChainProof` objects with caller-controlled public input.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
	    EIP-8025 proof gossip and recursive proof data model
	  </td>
      <td>
	    Gossip topics for execution proofs and beacon chain proofs;
        pubsub message handling and SSZ/Snappy codecs
      </td>
      <td>
	    BeaconChainProof containers and public input; recursive
        verification interface in ProofEngine; dummy proof harness
	  </td>
    </tr>
    <tr>
      <td nowrap>
      ...
      <td>
	    k-of-n ProofType threshold policy and per-payload proof
        aggregation; proof-feature configuration, observability, and
        integration tests
	  </td>
    </tr>
    <tr>
      ...
      <td>
	    Recursive proof verification and weak-subjectivity-to-head sync
	  </td>
      <td>
	    BeaconChainProof verification through ProofEngine; checkpoint
        anchor matching; sync gating from a verified proof
	  </td>
      <td>
	    Verified-head anchor advance; reorg handling; retention and
        pruning; restart re-derivation from gossip
	  </td>
    </tr>
    <tr>
      <td nowrap>
...
- Subscribe to and ingest recursive `BeaconChainProof` messages from
  the beacon chain proof gossip topic.
...
- Verify a single `BeaconChainProof` to establish execution validity
  from the trusted weak-subjectivity checkpoint to the head.
